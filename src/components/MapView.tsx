...
export default function MapView({ center }: { center?: [number, number] }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setMapError('Erro ao inicializar o mapa. Verifique as configurações.')
    }
  ...
  // Render error state
  if (mapError) {
    return <div id="map" ref={ref} style={{ width: '100%', height: '100%', minHeight: '70vh', borderRadius: 12, backgroundColor: '#ffeaea' }}>
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', color: '#d32f2f' }}>
        <div>{mapError}</div>
      </div>
    </div>
  }
  ...
  if (!isMapReady) {
    return <div id="map" ref={ref} style={{ width: '100%', height: '100%', minHeight: '70vh', borderRadius: 12, backgroundColor: '#f0f0f0' }}>
